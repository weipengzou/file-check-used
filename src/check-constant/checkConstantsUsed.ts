...
export const checkConstantsUsed = () => {
  type ValueType = ReturnType<typeof getTargetConstantArr>[0];
  const resSet = new Set<ValueType>(getTargetConstantArr()); // 目标文件下所有的文件数据
  ...
  allArr.forEach((filePath, index) => {
    ...
    resSet.forEach((item) => {
      const regex = new RegExp(`\\b${item.constant}\\b`, "g");
